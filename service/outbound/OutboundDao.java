...
public class OutboundDao extends BaseDao {
	 ...
	 */
	public OutboundDao() {
		insert.append("INSERT INTO outbound (cus_id,prod_id,price,num,total,status,remark) ");
		insert.append(" VALUES (:cus_id,:prod_id,:price,:num,:total,:status,:remark)");
	}
	...
	public int save(Outbound vo) {
		...
		sql.append("REPLACE INTO outbound (id,cus_id,prod_id,price,num,total,status,remark)");
		sql.append(" VALUES (?,?,?,?,?,?,?,?) ");
		Object[] params = {vo.getId(), vo.getCus_id(), vo.getProd_id(), vo.getPrice(), vo.getNum(), vo.getTotal(), vo.getStatus(), vo.getRemark()};
	...
	public int update(Outbound vo) {
		StringBuilder sql = new StringBuilder();
		sql.append("UPDATE outbound SET cus_id=?,prod_id=?,price=?,num=?,total=?,status=?,remark=? ");
		sql.append(" WHERE id=? ");
		Object[] params = {vo.getCus_id(), vo.getProd_id(), vo.getPrice(), vo.getNum(), vo.getTotal(), vo.getStatus(), vo.getRemark(), vo.getId()};
		return jdbcTemplate.update(sql.toString(), params);
	}
